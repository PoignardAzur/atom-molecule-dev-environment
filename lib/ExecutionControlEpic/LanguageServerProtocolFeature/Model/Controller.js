...
// Note: this one is mostly redundant with
// lib/ExecutionControlEpic/DevtoolLoadingFeature/Model/validateDevtool.js
...
function _checkStrategy(strategy) {
  if (strategy == null) {
  ...
  if (type !== "shell" && type !== "terminal" && type !== "node") {
    _throwError("strategy.type must be 'shell', 'terminal' or 'node'");
  }
  if (type === "node") {
    if (strategy.command != null) {
      _throwError("strategy cannot have 'command' field when type is 'node'");
    }
    if (strategy.path == null) {
      _throwError("strategy must have 'path' field when type is 'node'");
    }
  } else {
    if (strategy.path != null) {
      _throwError(`strategy cannot have 'path' field when type is '${type}'`);
    }
    if (strategy.args != null) {
      _throwError(`strategy cannot have 'args' field when type is '${type}'`);
    }
    if (strategy.command == null) {
      _throwError(`strategy must have 'command' field when type is '${type}'`);
    }
  }
}

function _checkController(controller) {
...
export function runController(config: {
  ...
  connection.onRequest("strategy/init", () => {
    ...
    try {
      ...
      _throwError(err.message);
    }

    _checkStrategy(execConfig.strategy);
    _checkController(execConfig.controller);
  ...
  connection.onRequest("initialize", async initializeOptions => {
    connection.onNotification("packages/didChange", ({ packages }) => {
      if (execConfig.controller.watchFiles == null) {
        return;
      }
      try {
