...
type AtomContext = any;
type MoleculeContext = any;
...
export default (context: ExecutionContext) => (
  action$: Observable,
  // TODO - MiddlewareAPI
  store: any,
) => {
    ...
    .mergeMap((action: RunTaskAction) => {
      return Rx.Observable.create(observer => {
        ...
        async function createLanguageClient() {
          ...
          const jsonrpcTrace$ = Rx.Observable.create(jsonObserver => {
          ...
          if (strategy.type === "terminal") {
            ...
            // TODO - check whether this code can be
            // called after a terminal is closed
            const { terminal, broker } = execution;
            connection.onNotification("terminal/output", ({ data }) => {
              terminal.write(data);
              broker.emit("terminal/output", { data });
            });
          }
        ...
        createLanguageClient().catch(err => {
